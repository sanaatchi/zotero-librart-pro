// @ts-nocheck
// Adapted from zotero-reference (AGPL-3.0) src/modules/views.ts
import { config } from "../../../package.json";
import { initReferenceLocale, getReferenceString } from "./referenceLocale";
import TipUI from "./tip";
import Utils from "./utils";
import LocalStorge from "./localStorage";
const localStorage = new LocalStorge(`${config.addonRef}-reference-cache`);
const getString = getReferenceString;
const initLocale = initReferenceLocale;
...
export default class Views {
                              ...
                              listeners: [
                                {
                                  ...
                                  listener: () => {
                                    ztoolkit.log(
                                      "dblclick: Copy all references",
                                    );
                                    const textArray: string[] = [];
                                    const labels = relatedbox.querySelectorAll(
                                      "#related-grid .box #reference-label",
                                    );
                                    ztoolkit.log(labels);
          ...
          window.setTimeout(async () => {
            if (Zotero.Prefs.get(`${config.addonRef}.autoRefresh`)) {
              const excludeItemTypes = (
                Zotero.Prefs.get(
                  `${config.addonRef}.notAutoRefreshItemTypes`,
                ) as string
              ).split(/,\s*/);
              if (panel.getAttribute("isAutoRefresh") != "true") {
                ...
                const itemType = Zotero.ItemTypes.getTypes().find(
                  (i) => i.id == id,
                )?.name as string;
  ...
  public refreshRelated(array: ItemBaseInfo[], node: HTMLDivElement) {
    ...
    array.forEach((info: ItemBaseInfo, i: number) => {
      const { box, label } = this.addRow(node, array, i, false, false) as any;
      if (!box) {
        return;
      }
  ...
  async loadingRelated() {
    ...
    const item = this.utils.getItem() as Zotero.Item;
    if (!item) {
      return;
    }
    ...
    const node = relatedbox.querySelector("#related-grid")
    ...
    const _relatedArray =
      ((await this.utils.API.getDOIRelatedArray(itemDOI)) as ItemBaseInfo[]) ||
      [];
    const func = relatedbox.refresh;
    relatedbox.refresh = () => {
      func.call(relatedbox);
      // #42，为Zotero相关条目添加悬浮提示
      // 把Zotero条目转化为Reference可识别形式
      node.querySelectorAll(".box").forEach((e: any) => {
  ...
  public async pdfLinks(
    ...
    while (
      !(
        ...
        (_pdfDocument = _window.PDFViewerApplication?.pdfDocument)
      )
    ) {
      await Zotero.Promise.delay(10);
    }
    ...
    const id = window.setInterval(async () => {
      ...
      _window.document
        .querySelectorAll(
          `section.linkAnnotation a[href^='#']:not([${config.addonRef}])`,
        )
        .forEach(async (a: any) => {
          const isClickLink = Zotero.Prefs.get(
            `${config.addonRef}.clickLink`,
          ) as boolean;
          const isHoverLink = Zotero.Prefs.get(
            `${config.addonRef}.hoverLink`,
          ) as boolean;
          let _a: any,
          ...
          if (isHoverLink) {
            ...
            _a.addEventListener("mouseenter", async (event: MouseEvent) => {
              ...
              // 确定 refIndex
              const distances = references.map(
                (ref: { x: number; y: number }) =>
                  (x - ref.x) ** 2 + (y - ref.y) ** 2,
              );
              const minDistance = [...distances].sort(
              ...
              if (reference) {
                timer = window.setTimeout(() => {
                  timer = undefined;
                  const rect = _a.getBoundingClientRect();
                  rect.y = rect.y + 40;
                  tipUI = this.showTipUI(rect, reference, "top center");
                }, 233);
              }
            });
  ...
  public async refreshReferences(
    ...
    const label = panel.querySelector("label#reference-num") as XUL.Label;
    ...
    if (source) {
      ...
      panel.setAttribute(
        "source",
        Zotero.Prefs.get(`${config.addonRef}.prioritySource`),
      );
    }
    ...
    panel
      .querySelectorAll("#zotero-reference-search")
      .forEach((e) => e.remove());
    ...
    if (panel.getAttribute("source") == "PDF") {
      ...
      if (references) {
        ...
        references = await this.utils.PDF.getReferences(
          reader,
          fromCurrentPage,
        );
      ...
      if (references) {
        ...
        if (this.utils.isDOI(DOI)) {
          ...
          popupWin
            ...
            .show();
          references = (await this.utils.API.getDOIInfoByCrossref(DOI))
            ?.references!;
        } else {
          if (!fileName) {
            try {
              const url = (await this.utils.API.getCNKIURL(title)) as string;
              if (url) {
          ...
          popupWin
            ...
            .show();
          references = (await this.utils.API.getCNKIFileInfo(fileName))
            ?.references!;
          if (!references) {
            popupWin.changeHeadline("[Fail] API");
            popupWin.changeLine({
              text: `Not Supported, ${fileName}`,
              type: "fail",
            });
    ...
    references.forEach(async (reference: ItemBaseInfo, refIndex: number) => {
      const { box } = this.addRow(panel, references, refIndex)!;
      // @ts-ignore
  ...
  public showTipUI(
    ...
    const toTimeInfo = (t: string) => {
      if (!t) {
        return undefined;
      }
      const info = new Date(t).toString().split(" ");
      return `${info[1]} ${info[3]}`;
    };
    const tipUI = new TipUI();
    tipUI.onInit(refRect, position);
    const refText = reference.text!;
    const getDefalutInfoByReference = async () => {
      ...
      if (localItem) {
        info = {
          ...
          tags: localItem.getTags().map((i: any) => {
            const ctag: any = localItem
              .getColoredTags()
              .find((ci: any) => ci.tag == i.tag);
            if (ctag) {
        ...
        const url = this.utils.identifiers2URL(info.identifiers);
        if (url) {
          info.url = url;
        }
      }
    ...
    const sourceConfig = {
      ...
      readpaper: {
        color: "#1f71e0",
        tip: "论文阅读平台ReadPaper共收录近2亿篇论文、2.7亿位作者、近3万所高校及研究机构，几乎涵盖了全人类所有学科。科研工作离不开论文的帮助，如何读懂论文，读好论文，这本身就是一个很大的命题，我们的使命是：“让天下没有难读的论文”",
      },
      semanticscholar: {
        color: "#1857b6",
      ...
      CNKI: {
        color: "#1b66e6",
        tip: "中国知网知识发现网络平台—面向海内外读者提供中国学术文献、外文文献、学位论文、报纸、会议、年鉴、工具书等各类资源统一检索、统一导航、在线阅读和下载服务。",
      },
    };
    for (let i = 0; i < coroutines.length; i++) {
      // 不阻塞
      window.setTimeout(async () => {
        const info = await coroutines[i];
        if (!info) {
          return;
        }
        const tagDefaultColor = "#59C1BD";
        const tags =
          (info.tags!.map((tag: object | string) => {
        ...
        if (info.identifiers.arXiv) {
          ...
          tags.push({
            text: "arXiv",
            color: sourceConfig.arXiv.color,
            tip: arXiv,
            url: info.url,
          });
        }
        if (info.identifiers.CNKI) {
          const url = info.identifiers.CNKI;
          tags.push({
            text: "URL",
            color: sourceConfig.CNKI.color,
            tip: url,
            url: info.url,
          });
        }
        ...
        tipUI.addTip(
          ...
          [
            ...
            [
              info?.primaryVenue,
              toTimeInfo(info.publishDate as string) || info.year,
            ]
              .filter((e) => e)
              .join(" \u00b7 "),
  ...
  public addRow(
    ...
    let notInLibarayOpacity: string | number = Zotero.Prefs.get(
      `${config.addonRef}.notInLibarayOpacity`,
    ) as string;
    ...
    // 避免重复添加
    const toText = (s: string) => s.replace(/[^\u4e00-\u9fa5a-zA-Z0-9]/g, "");
    if (
      [...node.querySelectorAll(".box label")].find(
        (e: any) => toText(e.innerText) == toText(refText),
      )
    ) {
      return;
    }
    ...
    const idText =
      (reference.identifiers &&
        ...
        Object.keys(reference.identifiers)[0] +
          ": " +
          Object.values(reference.identifiers)[0]) ||
      "Reference";
    // 当前item
    const item = this.utils.getItem()!;
          ...
          listener: async (event: any) => {
            ...
            if (event.ctrlKey || event.metaKey) {
              ...
              if (reference._item) {
                ...
                const item = await this.utils.searchLibraryItem(reference);
                if (item) {
              ...
              if (!URL) {
                const refText = reference.text!;
                const info: ItemBaseInfo = this.utils.refText2Info(refText);
                const popupWin = new ztoolkit.ProgressWindow("Searching URL", {
                  closeTime: -1,
                })
                  .createLine({
                    text: `Title: ${reference.title}`,
                    type: "default",
                  })
                  .show();
                if (this.utils.isChinese(refText)) {
                  ...
                  const DOI = (
                    await this.utils.API.getTitleInfoByConnectedpapers(
    ...
    const label = ztoolkit.UI.createElement(document, "label", {
      id: "add-remove",
      namespace: "xul",
      attributes: {
        value: "+",
      },
      classList: ["zotero-clicky", "zotero-clicky-plus"],
    });

    const enterEdit = () => {
      const label = box.querySelector("#reference-label")! as XUL.Label;
      label.style.display = "none";
      const textarea = ztoolkit.UI.createElement(document, "textarea", {
        id: "reference-edit",
        namespace: "html",
        attributes: {
          flex: "1",
      ...
      const exitEdit = async () => {
        ...
        const i = this.utils.searchLibraryItem(reference);
    ...
    const remove = async () => {
      ...
      const popunWin = new ztoolkit.ProgressWindow("Removing Item", {
        closeTime: -1,
      })
      ...
      const relatedItem = this.utils.searchRelatedItem(
        item,
        reference._item,
      ) as Zotero.Item;
      if (!relatedItem) {
    ...
    const add = async (collections: undefined | number[] = undefined) => {
      ...
      // 检查本地
      let refItem =
        reference._item || (await this.utils.searchLibraryItem(reference));
      // 禁用按钮
      setState();
      if (refItem) {
        ...
        const info: ItemBaseInfo = this.utils.refText2Info(reference.text!);
        // 知网
        ...
        // DOI or arXiv
          ...
          if (Object.keys(reference.identifiers).length == 0) {
            ...
            popupWin.changeLine({
              text: collapseText(`Title: ${info.title!}`),
            });
            const DOI = (
              await this.utils.API.getTitleInfoByConnectedpapers(info.title)
            )?.identifiers.DOI as string;
            if (!this.utils.isDOI(DOI)) {
              setState("+");
              popupWin.changeLine({ type: "fail" });
              popupWin.startCloseTimer(3000);
          ...
          popupWin.changeLine({
            text: collapseText(
              `${Object.keys(reference.identifiers)}: ${Object.values(reference.identifiers)[0]}`,
            ),
          });
          // done
        ...
        for (const collectionID of collections || item.getCollections()) {
    ...
    const updateRowByItem = (refItem: Zotero.Item) => {
      ...
      const alreadyRelated = this.utils.searchRelatedItem(item, refItem);
      if (alreadyRelated) {
        setState("-");
      }
    };
    ...
    if (notInLibarayOpacity < 1) {
      window.setTimeout(async () => {
        const refItem =
          reference._item ||
          ((await this.utils.searchLibraryItem(reference)) as Zotero.Item);
        if (refItem) {
          updateRowByItem(refItem);
        }
      }, refIndex * 0);
    ...
    box.addEventListener("mouseenter", () => {
      ...
      const timeout = parseInt(
        Zotero.Prefs.get(
          `${config.addonRef}.showTipAfterMillisecond`,
        ) as string,
      );
      const position =
    ...
    if (
      addSearch &&
      referenceNum &&
      !node.querySelector("#zotero-reference-search")
    ) {
      this.addSearch(node);
    }
    // 高度
    const relatedGrid = node.querySelector("#related-grid") as HTMLDivElement;
    relatedGrid.style.maxHeight = `${
      document.documentElement.getBoundingClientRect().height -
      relatedGrid.getBoundingClientRect().top
    }px`;
    return { box, label };
  }
              ...
              },
              {
                ...
                listener: async () => {
                  ...
                  node
                    ...
                    .forEach((e: any) => (e.style.display = ""));
                  if (keywords.length == 0) {
                    return;
                  }
                  node
                    .querySelectorAll("#related-grid .box")
                    .forEach((box: any) => {
                      const content = (
                        box.querySelector("#reference-label") as any
                      ).textContent;
                      ...
                      for (let i = 0; i < keywords.length; i++) {
                        isAllMatched =
                          ...
                          content
                            .toLowerCase()
                            .indexOf(keywords[i].toLowerCase()) >= 0;
                      }
                      if (isAllMatched) {
