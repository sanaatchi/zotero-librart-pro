// @ts-nocheck
// Adapted from zotero-reference (AGPL-3.0) src/modules/tip.ts
...
export default class TipUI {
  ...
  constructor() {
    this.shadeMillisecond = parseInt(
      Zotero.Prefs.get(`${config.addonRef}.shadeMillisecond`) as string,
    );
    this.removeTipAfterMillisecond = parseInt(
      Zotero.Prefs.get(
        `${config.addonRef}.removeTipAfterMillisecond`,
      ) as string,
    );
  ...
  public clear() {
    document
      .querySelectorAll(".zotero-reference-tip-container")
      .forEach((e: any) => {
        e.style.opacity = "0";
        window.setTimeout(() => {
          e.remove();
  ...
  private place() {
    `
		...
		右上(x=0, y=0)
		`;
    const setStyles = (styles: { [key: string]: string }) => {
  ...
  private buildContainer() {
    // 位置计算
    this.container = ztoolkit.UI.createElement(document, "div", {
      namespace: "html",
      ...
      styles: {
        ...
        padding: "1em",
        backgroundColor: Zotero.Prefs.get(
          `${config.addonRef}.tipBackgroundColor`,
        ) as string,
        opacity: "0",
  ...
  public addTip(
    ...
    const translateNode = async function (event: any) {
      if (
    ...
    const contentNode = ztoolkit.UI.createElement(document, "div", {
      ...
      subElementOptions: [
        {
          ...
          styles: {
            ...
            marginBottom: ".25em",
            fontSize: "1.2em",
            color: Zotero.Prefs.get(
              `${config.addonRef}.tipTitleColor`,
            ) as string,
          },
          directAttributes: {
            innerText: title,
    ...
    const optionNode = ztoolkit.UI.createElement(document, "div", {
      id: `option-${index}`,
      styles: {
        ...
        backgroundColor: isSelect
          ? this.option.color.active
          : this.option.color.default,
      ...
      listeners: [
        {
          ...
          listener: () => {
            ...
            Zotero.Prefs.set(
              `${config.addonRef}.${according}InfoIndex`,
              index!,
            );
            this.place();
          },
        },
        {
          type: "mouseenter",
          listener: () => {
            const tag = tags.find((tag) => tag.source);
            const source =
              (tag &&
                tag.source &&
                according &&
                `${tag.source} view according to ${according}`) ||
              "reference view";
            Zotero.ProgressWindowSet.closeAll();
  ...
  private zoom(event: any) {
    const _scale = this.container.style.transform.match(/scale\((.+)\)/);
