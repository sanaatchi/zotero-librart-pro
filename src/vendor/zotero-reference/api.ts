// @ts-nocheck
// Adapted from zotero-reference (AGPL-3.0) src/modules/api.ts
...
class API {
  ...
  constructor(utils: Utils) {
    ...
    this.Info = {
      crossref: (item: any) => {
        ...
        const references: ItemBaseInfo[] = item.reference?.map((item: any) => {
          let identifiers;
          ...
          if (item.unstructured) {
            ...
            if (item["article-title"] && item.year && item.author) {
        ...
        const info: ItemInfo = {
          ...
          publishDate:
            item.published && item.published["date-parts"][0].join("-"),
          source: item.source.toLowerCase(),
          primaryVenue: item["container-title"]
      ...
      readpaper: (data: any) => {
        const info: ItemInfo = {
          ...
          authors: data?.authorList.map((i: any) =>
            this.utils.Html2Text(i.name),
          ),
      ...
      arXiv: (data: any) => {
        const info: ItemInfo = {
          ...
          primaryVenue:
            data["arxiv:comment"] &&
            data["arxiv:comment"][0]["_"].replace(/\n/g, ""),
        };
        return info;
      },
    };
    this.BaseInfo = {
      readcube: (data: any) => {
        ...
        const url = this.utils.identifiers2URL(identifiers);
        const related: ItemBaseInfo = {
  ...
  async getDOIBaseInfo(DOI: string): Promise<ItemBaseInfo | undefined> {
    ...
    for (const route in routes) {
      const response = await this.requests.get(routes[route]);
      if (response) {
        response.DOI = DOI;
        return this.Info[route as keyof typeof this.Info](
          response,
        ) as ItemBaseInfo;
      }
    }
   ...
   * @param DOI
   */
  async getDOIInfoBySemanticscholar(
    DOI: string,
  ...
  async getDOIInfoByCrossref(DOI: string): Promise<ItemInfo | undefined> {
    ...
    if (response) {
      response.DOI = DOI;
      const info: ItemInfo = this.Info.crossref(response);
      return info;
    }
  }
  ...
  async getDOIRelatedArray(
    DOI: string,
    limit: number = 20,
  ): Promise<ItemBaseInfo[] | undefined> {
    const res = await this.requests.get(
      `https://rest.connectedpapers.com/id_translator/doi/${DOI}`,
      "json",
      {
        "user-agent":
          "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 e/107.0.0.0 Safari/537.36",
      },
    );
    ...
    const response = await this.requests.get(api, "json", {
      cookie:
        "aws-waf-token=fcf9f43b-d494-44a8-8806-da20c50d9457:AQoAaIgZ+Q0AAAAA:z/ZtlDV2Oz/Ymw+RFbJ0vnEAl1/wBKTH6I4/INUou3Qqkm00bibIWkYKq0w3qq4yxB2EtdBTtRT7Q2MBPjx17WmPmcVznf7mTMTwFQjmJOB2VgQeoBzsmuzVlI/l/NBlyTFdH8xEKYYWbXB8R5oK9o7JxolugTzDKvLX4Pc57cdkbCA5A6AIExi/Wm16",
    });
    ztoolkit.log(response);
    if (response) {
      const arr: ItemInfo[] = response.papers.map((i: any) => {
        const info: ItemInfo = {
          title: i.title.text,
          identifiers: {},
          year: i.year.text,
  ...
  async _getDOIRelatedArray(
    ...
    const api = `https://www.semanticscholar.org/api/1/search/paper/${res.paperId}/citations`;
    const response = await this.requests.post(api, {
      page: 1,
      pageSize: 20,
      sort: "relevance",
      authors: [],
    ...
    ztoolkit.log(response);
    if (response) {
      const arr: ItemInfo[] = response.results.map((i: any) => {
        const info: ItemInfo = {
          title: i.title.text,
          identifiers: {},
          year: i.year,
  ...
  async getArXivInfo(arXiv: string) {
    ...
    const response = await this.requests.get(api, "application/xhtml+xml");
    if (response) {
  ...
  async getTitleInfoByCrossref(title: string): Promise<ItemInfo | undefined> {
    ...
    if (response) {
      ...
      const item = response.message.items.filter(
        (e: any) => skipTypes.indexOf(e.type) == -1,
      )[0];
      const info = this.Info.crossref(item) as ItemInfo;
      return info;
    }
  }

  async getTitleInfoByConnectedpapers(
    text: string,
  ): Promise<ItemInfo | undefined> {
    let title = text;
    ...
    if (response) {
      if (response?.results?.length) {
        ...
        const info = this.Info.connectedpapers(item) as ItemInfo;
        return info;
      }
    }
  }

  async getTitleInfoByReadpaper(
    title: string,
    body: object = {},
    ...
    const response = await this.requests.post(api, body);
    if (response && response?.data?.list?.[0]) {
      const data = response?.data?.list?.[0];
      // 验证DOI
      if (doi) {
        // 获取paperId的doi
        const _res = await this.requests.post(
      ...
      const info = this.Info.readpaper(data) as ItemInfo;
      if (doi) {
  ...
  async _getCNKIURL(title: string, author: string) {
    ...
    let cnkiURL;
    const oldFunc = Zotero.Jasminum.Scrape.getItemFromSearch;
    ztoolkit.patch(
      ...
      (original) => (arg: any) => {
        ...
        ztoolkit.log(text);
        text = escape(unescape(text).replace(/SCDB/g, "CFLS"));
        ztoolkit.log(text);
        return text;
      },
    );
    Zotero.Jasminum.Scrape.getItemFromSearch = function (htmlString: string) {
      try {
        const res = htmlString.match(
          /href='(.+FileName=.+?&DbName=.+?)'/i,
        ) as any[];
        if (res.length) {
          return res[1];
        }
    ...
    const args = this.utils.parseCNKIURL(cnkiURL);
    if (args) {
  ...
  async getCNKIURL(keywords: string, slience: boolean = false) {
    if (!slience) {
      new ztoolkit.ProgressWindow("[Pending] API", {
    ...
    try {
      if (res) {
        let cnkiURL = res.responseText.match(
          /href='(.+FileName=.+?&DbName=.+?)'/i,
        );
        if (cnkiURL) {
          const args = this.utils.parseCNKIURL(cnkiURL[1]);
          if (args) {
            cnkiURL = `https://kns.cnki.net/kcms/detail/detail.aspx?FileName=${args.fileName}&DbName=${args.dbName}&DbCode=${args.dbCode}`;
            return cnkiURL;
          }
  ...
  async getTitleInfoByCNKI(refText: string): Promise<ItemInfo | undefined> {
    ...
    const res = this.utils.parseRefText(refText);
    ...
    const url = await this.getCNKIURL(res.title, true);
    if (!url) {
      return;
    }

    const htmlString = await this.requests.get(url, "text");
    ztoolkit.log(url, htmlString);
    const parser = ztoolkit.getDOMParser();
    const doc = parser.parseFromString(htmlString, "text/html")
      .childNodes[1] as any;
    const aTags = doc.querySelectorAll(".top-tip span a");
    const info: ItemInfo = {
      identifiers: { CNKI: url },
      title: doc.querySelector(".brief h1").innerText,
      abstract: doc.querySelector("span#ChDivSummary").innerText,
      authors: [...doc.querySelectorAll("#authorpart span a")].map(
        (a) => a.innerText,
      ),
      type: "journalArticle",
      ...
      url: url,
      source: "CNKI",
      tags: [...doc.querySelectorAll(".keywords a")]
  ...
  async getCNKIFileInfo(
    ...
    const username = Zotero.Prefs.get(
      `${config.addonRef}.CNKI.username`,
    ) as string;
    const password = Zotero.Prefs.get(
      `${config.addonRef}.CNKI.password`,
    ) as string;
    ...
    if (String(refData.code) != "200") {
      if (count < 3) {
        ...
        new ztoolkit.ProgressWindow("[Fail] API", {
          ...
          .createLine({
            text: `${refData.code}: ${refData.promptMessage}`,
            type: "fail",
          })
          .show();
        return;
    ...
    if (String(infoData.code) == "200") {
      infoData.content.paper.bibliography.forEach((ref: { title: string }) => {
        const _ref = refData.content.refer.find((_ref: { title: string }) => {
      ...
      refData.content.refer
        .sort(
          (a: any, b: any) =>
            Number(a.citationNumber) - Number(b.citationNumber),
        )
        .forEach(
          (ref: {
            type: string;
