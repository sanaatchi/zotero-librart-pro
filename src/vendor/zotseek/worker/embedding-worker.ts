...
async function initPipeline(): Promise<void> {
  ...
  if (!CURRENT) {
    ...
    postMessage({
      type: "error",
      error:
        "Model config missing -- send init message with model config first",
    });
    return;
  }
  ...
  postMessage({
    type: "status",
    status: "loading",
    message: `Loading model on ${deviceLabel}...`,
  });
  ...
  if (useWebGPU) {
    try {
      ...
      postMessage({
        type: "status",
        status: "ready",
        message: `Model loaded on GPU (${loadTime}ms)`,
      });
      isLoading = false;
      return;
    } catch (error: any) {
  ...
  try {
    ...
    postMessage({
      type: "status",
      status: "ready",
      message: `Model loaded on CPU (${loadTime}ms)`,
    });
  } catch (error: any) {
    const loadTime = Date.now() - startTime;
  ...
  switch (type) {
    ...
    case "embed":
      ...
      if (embeddingPipeline) {
        ...
        const kind: "query" | "doc" =
          data?.kind ?? (data?.isQuery ? "query" : "doc");
...
postMessage({
  ...
  level: "info",
  message:
    "Embedding worker initialized (awaiting model config via init message)",
  data: { maxChars: MAX_CHARS, webGPUAvailable: hasWebGPU },
});
postMessage({
  type: "status",
  status: "initialized",
  message: `Worker loaded (WebGPU ${hasWebGPU ? "detected" : "not available"})`,
});
