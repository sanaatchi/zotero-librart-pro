...
connection-map-kutuphane-graph-warn-low = 警告：仅 { $inScope }/{ $total } 条离线边匹配当前范围 — 缩小范围或重建图
connection-map-kutuphane-graph-warn-stale = 警告：离线图已 { $days } 天 — 运行 python _build_zotero_connection_graph.py
...
connection-map-status-updated = { $library } · 已更新 { $time } · { $confirmed } 已确认，{ $suggested } 建议
connection-map-scope-selection = 范围：所选条目（+相关）· { $shown } / { $total }
connection-map-scope-collection = 范围：“{ $name }” · { $shown } / { $total }
connection-map-scope-library-cap = 范围：文库抽样 · { $shown } / { $total } — 请选择集合或条目以缩小范围
connection-map-scope-truncated-hint = 非完整文库
