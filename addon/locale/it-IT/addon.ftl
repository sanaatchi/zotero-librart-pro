...
menu-markdb-scan = Scansiona vault + sync tag
menu-markdb-open = Apri nota in Obsidian
...
markdb-error-open = Impossibile aprire URI Obsidian.
markdb-error-no-selection = Seleziona prima un elemento.
markdb-error-no-note = Nessuna nota vault per questo elemento. Esegui la scansione.
markdb-scan-done = MarkDB: { $total } note, { $matched } con chiave · tag +{ $tagged } / −{ $removed }
...
connection-map-kutuphane-graph-meta = Kutuphane graf: { $inScope }/{ $total } kenar (kapsamda) · { $at }
connection-map-kutuphane-graph-warn-low = Avviso: solo { $inScope }/{ $total } archi offline in questo ambito — restringi o rigenera il grafo
connection-map-kutuphane-graph-warn-stale = Avviso: grafo offline di { $days } giorni — esegui python _build_zotero_connection_graph.py
